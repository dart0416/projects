...
// Function to decrypt a single file (for simplicity, it calls encryptFile with the same logic)
void decryptFile(const char *inputFilename, const char *outputFilename, char key) {
    encryptFile(inputFilename, outputFilename, key);
}

// Generic function to process files in a folder based on the provided processFile function (either encrypt or decrypt)
void processFilesInFolder(const char *inputFolder, const char *outputFolder, char key, void (*processFile)(const char *, const char *, char)) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_REG) { // Check if it's a regular file
            ...
            // Dynamically allocate memory for input and output paths
