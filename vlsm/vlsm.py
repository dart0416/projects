# Function to convert IP address from string format to a 32-bit integer
def ip_to_int(ip):
    octets = map(int, ip.split('.'))
    return sum([octet << (8 * (3 - i)) for i, octet in enumerate(octets)])

# Function to convert a 32-bit integer to an IP address in string format
def int_to_ip(int_val):
    return '.'.join(str((int_val >> (8 * i)) & 0xFF) for i in reversed(range(4)))

# Function to calculate the broadcast address given the network address and subnet mask
...
# Function to calculate the network address given the IP address and subnet mask
...
# Function to generate a subnet mask from a prefix length (CIDR notation)
...
# Function to convert a subnet mask from dot-decimal format to prefix length (CIDR notation)
def netmask_to_cidr(netmask):
    return sum([bin(int(x)).count('1') for x in netmask.split('.')])

# Function to parse the input mask, allowing both dot-decimal and CIDR notation
def parse_mask(input_mask):
    if '/' in input_mask:
        # Extract the prefix length from CIDR notation (e.g., "/24").
        prefix_length = int(input_mask.strip('/'))
        # Convert the prefix length back into a subnet mask for further processing.
        return subnet_mask_from_prefix_length(prefix_length), prefix_length
    else:
        return input_mask, netmask_to_cidr(input_mask)

# Function to calculate subnets based on the base network, subnet mask, and sizes needed for each subnet
def calculate_subnets(base_network, base_mask, sizes):
    sizes.sort(reverse=True)  # Sort sizes in descending order to allocate largest subnets first
    subnets = []
...
# Main function to gather input and display calculated subnet information
def main():
